...
| ID | Requirement | Verification |
|----|-------------|--------------|
| DD-045 | When dropd runs for the first time for a given playlist definition, dropd shall create a new playlist in the user's Apple Music library with the configured playlist name. | Test: Configure a new playlist definition, trigger a sync, and verify the playlist is created in Apple Music. |
...
| DD-047 | When dropd updates a playlist, dropd shall not add tracks that are already present in that playlist. | Test: Trigger two consecutive syncs with the same new releases and verify no duplicate tracks are added. |
| DD-048 | When dropd updates a playlist, dropd shall remove tracks whose release date is older than the configured rolling window duration. | Test: Set the rolling window to 14 days, add a track released 15 days ago, trigger a sync, and verify the track is removed. |
...
- The `genreNames` attribute on Apple Music albums/songs provides sufficient granularity to classify releases into user-configured genre playlists (for example, distinguishing "Electronic" from "Hip-Hop/Rap" from "Jazz").
